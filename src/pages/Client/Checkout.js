...
export default function(props) {
  ...
  }, [user.city, order.city, totalPrice, user.id, user.address, user.district, user.email, user.name, user.phone])
  ...
  const handleSubmit = async (event) => {
    ...
    if (order.payment === 'cash') {
      ...
      const data = {
        ...
        customer_province: order.district,
        customer_district: order.district,
        lines: order.cartItems.map(item => ({
          product: item.id, 
          quantity: item.quantity
        })),
        customer_ward: order.city,
        customer_address: order.address,
        total_amount: order.totalPrice,
        payment_method: "CASH",
        creator: order.id
      }
      const res = await axios.post("http://127.0.0.1:8000/api/orders/", data, { headers: {"Authorization" : `Bearer ${token}`}});
      createOrder(res.data);
      localStorage.removeItem('cartItems');
      setStateDefault();
      props.history.push(`/order-received/${res.data.id}`);
    } 
          ...
          <Form onSubmit={handleSubmit}>
            <div className="billing-address">
              ...
              <FormGroup>
                ...
                <Input 
                  type="text" 
                  value={order.name || `${user.first_name} ${user.last_name}` || ''} 
                  name="name" 
