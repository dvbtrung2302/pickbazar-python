...
export class AdminProvider extends React.Component {
  constructor(props) {
    ...
    this.state = {
      ...
      refresh: localStorage.getItem('refresh_adminToken') || '',
      orders: [],
      users: [],
      products: [],
      promotions: [],
  ...
  render() {
    ...
    if (filter.keyword) {
      filtedProducts = filtedProducts.filter(product => {
        return product.name.toLowerCase().indexOf(filter.keyword.toLowerCase()) !== -1;
      })
    }
